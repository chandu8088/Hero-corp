...
@Component(service = Servlet.class, immediate = true, property = {
        Constants.SERVICE_DESCRIPTION + "=Exchange Journey Model List servlet",
        ServletResolverConstants.SLING_SERVLET_RESOURCE_TYPES + "=hero-aem-website/components/content/exchangeformsteps",
        ServletResolverConstants.SLING_SERVLET_SELECTORS + "=exchangemodellist",
        ServletResolverConstants.SLING_SERVLET_EXTENSIONS + "=json",
        ServletResolverConstants.SLING_SERVLET_METHODS + "=" + HttpConstants.METHOD_POST })
public class ExchangeModelListServlet extends SlingAllMethodsServlet {
    /**
     * Generated serialVersionUID
     */
    private static final long serialVersionUID = 4438376868274173005L;
    private static final Logger log = LoggerFactory.getLogger(ExchangeModelListServlet.class);
    ...
    protected void doPost(final SlingHttpServletRequest req, final SlingHttpServletResponse resp)
        ...
        try {
            HttpPost httpPost = new HttpPost(apiEndpointConfigurations.getConfigProperty("getExchangeModelAPI"));
            List < NameValuePair > form = new ArrayList < > ();
            form.add(new BasicNameValuePair("make", req.getParameter("make")));
            form.add(new BasicNameValuePair("category", req.getParameter("category")));
            ...
            log.info("Exchange Journey Model List Response" + jsonVal);
            resp.getWriter().write(jsonVal);
        } catch (IOException e) {
            log.error("Exception in Exchange Journey Model List Servlet" + e.getMessage());
        } finally {
            httpClient.close();
        }
    }
