...
public class BookTestRideServlet extends SlingAllMethodsServlet {
	...
	protected void doPost(final SlingHttpServletRequest req, final SlingHttpServletResponse resp)
		...
		try {
			...
			if(variationType.equalsIgnoreCase("campaignpage") && StringUtils.isNotBlank(leadGenerationAPIBean.getDealer_code()) && (leadGenerationAPIBean.getBike_model().equals("XOOM") || leadGenerationAPIBean.getBike_model().equals("Passion+") || leadGenerationAPIBean.getBike_model().equals("Glamour")
			...
			if(validationFlag) {
				...
				if(variationType.equalsIgnoreCase("harleycampaign")) {
				...
				else if((variationType.equalsIgnoreCase("festiveappcampaign")) || (variationType.equalsIgnoreCase("campaignpage") && StringUtils.isNotBlank(leadGenerationAPIBean.getDealer_code()))){
					if((variationType.equalsIgnoreCase("festiveappcampaign")) || (leadGenerationAPIBean.getBike_model().equals("XOOM") || leadGenerationAPIBean.getBike_model().equals("Passion+") || leadGenerationAPIBean.getBike_model().equals("Glamour")
					|| leadGenerationAPIBean.getBike_model().equals("Super Splendor Xtec") || leadGenerationAPIBean.getBike_model().equals("Destini Prime") || leadGenerationAPIBean.getBike_model().equals("Pleasure+ Xtec")
					|| leadGenerationAPIBean.getBike_model().equals("Xtreme 160R 4V") || leadGenerationAPIBean.getBike_model().equals("Xpulse 200 4V") || leadGenerationAPIBean.getBike_model().equals("Destini 125 Xtec"))) {
						String xml = soapServiceGen.GenerateXML(leadGenerationAPIBean);
						log.info("XOOM South Region/Passion plus Campaign XML"+ xml);
						String xoomSouthRegionResp = soapServiceGen.ProcessSOAPRequest(xml);
						log.info("XOOM South Region/Passion plus Campaign Response"+ xoomSouthRegionResp);
					...
					try {
						...
						log.error("Exception in NDC Service campaign Servlet"+e.getMessage());
					}
					finally {
						httpClient.close();
					}
					...
					//Generate Lead Sequence Number
					String leadSequenceNumber = FormsUtils.generateLSequenceNumber();
...
//					CloseableHttpResponse response = httpClient.execute(postReq);
					...
					try {
						...
						requestMap.put("mx_Form_Field_10", leadSequenceNumber);
						StringSubstitutor sub = new StringSubstitutor(requestMap);
						...
						if (jsonVal.toLowerCase().contains("success")) {
							obj.addProperty("leadSequenceNumber", leadSequenceNumber);
							obj.addProperty("message", "success");
						} else {
							obj.addProperty("message", "failure");
						}
			...
			log.info("Exception in SOAP Harley Form"+ e);
		} finally {
			httpClient.close();
		}
	}
