$(document).ready(function() {
        ...
        if ($('.sideNavbar-links .profilesection').length > 0 && getCookie('data') != null) {
            let userData = JSON.parse(atob(getCookie('data')));
            vehicleCallbackSucess(userData, $('.profilesection'), $('.userDetailsNavTemplate'));
            showInitials();
        }

        let userNameMobile = toTitleCase($('.profile-update .myvehicle-profile-pic h4').eq(0).text());
        $('.profile-update .myvehicle-profile-pic h4').text(userNameMobile);
