$(document).ready(function() {
    ...
    function validateMobileNo(otpElement) {
        ...
        sendAjaxCall(window.aemConfigs.apiEndpoints.middleWareAPI, dmsRequest, checkDMSRow).then(function(data) {
        	if(data.PWSESSIONRS[0].PWPROCESSRS.PWERROR == '') {
	        	if (data.PWSESSIONRS[0].PWPROCESSRS.PWDATA.oa_verifyDMS.Row[0].status) {
	                var sendOTPRow = [{
	                    "0": "+91",
    ...
    function getAccessToken(loginElement) {
        ...
        let mobileNumber = $("#loginForm").find("[name='loginMobileNo']").val();
        let loginOTP = $("#loginForm").find("[name='loginOtp']").val();
        if(!mobileNumber){
        ...
        sendAjaxCall(window.aemConfigs.apiEndpoints.middleWareAPI, getTokenRequest, getTokenRow).then(function(data) {
        	if(data.PWSESSIONRS[0].PWPROCESSRS.PWERROR == '') {
	            if (data.PWSESSIONRS[0].PWPROCESSRS.PWDATA.MB_GET_AUTHORISED.Row[0].status == "success") {
	                ...
	                document.cookie = "data=" + btoa(JSON.stringify(dataObject)) + "; path=/";
	                ...
	                sendAjaxCall(window.aemConfigs.apiEndpoints.middleWareAPI, getDetailsDMSRequest, getDetailsDMSRow).then(function(data) {
	                    if (data.PWSESSIONRS[0].PWPROCESSRS.PWDATA.oa_getDetailsDMS.Row.length > 0) {
                            ...
                            userObject.state = data.PWSESSIONRS[0].PWPROCESSRS.PWDATA.oa_getDetailsDMS.Row[0].STATE,
                            userObject.city = data.PWSESSIONRS[0].PWPROCESSRS.PWDATA.oa_getDetailsDMS.Row[0].CITY,
                            userObject.username = data.PWSESSIONRS[0].PWPROCESSRS.PWDATA.oa_getDetailsDMS.Row[0].name;
                            document.cookie = "exchangeloggeddetails=" + btoa(JSON.stringify(userObject)) + "; path=/";
    ...
    function getVehicleData() {
        ...
        sendAjaxCall(window.aemConfigs.apiEndpoints.middleWareAPI, data, getRow).then(function(data) {
            ...
            for (var i = 0; i < getVehicleDetails.length; i++) {
                ...
                vehicleArray.category_code = getVehicleDetails[i].category_code;
            ...
            if(window.location.href.indexOf('/my-account/') != -1 || window.location.href.indexOf('/xclan/') != -1) {
            	location.reload();
...
$(document).ready(function() {
    ...
    if(cookieData){
    let userData = cookieData && JSON.parse(atob(getCookie('data')));
