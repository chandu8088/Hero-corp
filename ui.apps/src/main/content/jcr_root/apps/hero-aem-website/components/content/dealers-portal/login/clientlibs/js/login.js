$(document).ready(function () {
  var verificationAPI =
    "https://hmclmobazfun02p-staging.azurewebsites.net/api/cloudware_prod?ORG_ID=HMBPT";
  ...
  function sendOTP(otpElement) {
    ...
    sendAjaxCall(verificationAPI, sendOTPRequest, sendOTPRow).then(function (
      data
    ) {
      if (data.PWSESSIONRS[0].PWPROCESSRS.PWERROR == "") {
        if (
          data.PWSESSIONRS[0].PWPROCESSRS.PWDATA.MB_login_sms.Row[0].status ==
          "sent."
        ) {
  ...
  function getAccessToken(loginElement) {
    ...
    var getTokenRequest = createRequestBody(
      "MB_Verify_OTP_latest",
      getTokenRow,
      "1"
    );
    $("#hero-loader").removeClass("d-none");
    sendAjaxCall(verificationAPI, getTokenRequest, getTokenRow).then(function (
      data
    ) {
      if (data.PWSESSIONRS[0].PWPROCESSRS.PWERROR == "") {
        if (
          data.PWSESSIONRS[0].PWPROCESSRS.PWDATA.MB_Verify_OTP_latest.Row[0]
            .success == "1"
        ) {
          window.digitalData = {
          ...
          document.cookie =
            ...
            "; path=/; Secure";
          var concatData =
            responseData.dealer_code +
            "," +
            responseData.token +
            "," +
            responseData.area_office_code;

          $(".log-in-link").addClass("d-none");
          $(".logout-header").removeClass("d-none");
          document.cookie =
            "sessionStat=" + btoa(concatData) + "; path=/; Secure";
          window.location.href = $("#loginForm")
            .find('[name="loggedInPage"]')
            .val();
        } else if (
          data.PWSESSIONRS[0].PWPROCESSRS.PWDATA.MB_Verify_OTP_latest.Row[0]
            .success != "1"
