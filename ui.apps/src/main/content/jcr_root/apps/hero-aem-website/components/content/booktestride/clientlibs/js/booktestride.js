$(document).ready(function () {
  ...
  if (
    ...
    if ($(".book-test-drive-section").length) {
    ...
    $(
      ...
      if ("vehiclename" == button.attr("name")) {
        ...
        button.addClass('active');
        $('[name="docVal"]').val($(this).text().trim());
        if (checkHidden(form)) {
        ...
        success: function (resp) {
          if (JSON.parse(JSON.stringify(resp)).login == "true") {
            if ($(window).width() < 768) {
              ...
              $(currentForm)
                .closest(".book-test-drive-wrapper")
                .find(".book-test-drive-parent:not(.modal-body .book-test-drive-parent) .book-test-drive-form")
                .addClass("d-none");
            }
            $(currentForm)
              .closest(".book-test-drive-wrapper")
